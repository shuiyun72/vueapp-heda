 ...
 * @param fmt
 */
dateHelper.format = function(date, fmt) {
  let o = {
...
dateHelper.formatToDate = function(dateStr) {
...
dateHelper.getDateStart = function(date) {
  let fmt = "yyyy-MM-dd";
  let dateStartStr = this.getDateStartStr(date, fmt);
  let startTime = new Date(Date.parse(dateStartStr));
  return startTime;
};

dateHelper.getDateStartStr = function(date, fmt) {
...
dateHelper.getDateEnd = function(date) {
  let fmt = "yyyy-MM-dd";
  let dateEndStr = this.getDateEndStr(date, fmt);
  let endTime = new Date(Date.parse(dateEndStr));
  return endTime;
};

dateHelper.getDateEndStr = function(date, fmt) {
  if (typeof fmt == "undefined") {
    fmt = "yyyy-MM-dd";
  }
...
dateHelper.compareDate = function(d1, d2) {
  if (d1 && d2) {
...
dateHelper.isLeapYear = function(date) {
...
dateHelper.isValidDate = function(dateStr) {
  if (this.isNotEmpty(dateStr)) {
...
dateHelper.addDay = function(date, dayNum) {
  if (
    this.isNotEmpty(date) &&
    this.isNotEmpty(dayNum) &&
...
dateHelper.addDayStr = function(dateStr, dayNum) {
  let date = "";
  if (
    this.isNotEmpty(dateStr) &&
    this.isNotEmpty(dayNum) &&
    typeof dayNum == "number"
  ) {
...
dateHelper.addMonth = function(date, monthNum) {
  if (
    this.isNotEmpty(date) &&
    this.isNotEmpty(monthNum) &&
    date instanceof Date &&
...
dateHelper.addMonthStr = function(dateStr, monthNum) {
  let date = "";
  if (
    this.isNotEmpty(dateStr) &&
    this.isNotEmpty(monthNum) &&
...
dateHelper.addYear = function(date, yearNum) {
  if (
    this.isNotEmpty(date) &&
    this.isNotEmpty(yearNum) &&
    date instanceof Date &&
...
dateHelper.addYearStr = function(dateStr, yearNum) {
  let date = "";
  if (
    this.isNotEmpty(dateStr) &&
    this.isNotEmpty(yearNum) &&
...
dateHelper.isNotEmpty = function(str) {
...
dateHelper.getWeek = function(date, type) {
  if (date) {
