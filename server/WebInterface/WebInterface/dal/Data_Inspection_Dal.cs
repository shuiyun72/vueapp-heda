...
namespace WebInterface.dal
{
    public static class Data_Inspection_Dal
    {
        ...
        public static string User_CheckLogin(string name, string pwd, string smid)
        {
            ...
            try
            {
                string sql = string.Format(@"select iAdminID as PersonId,iDeptID as DepartmentId, cAdminName as PersonName,cAdminPassWord as PassWord,cDepName as DeptName,'巡检员' RoleName,Smid,1 as IsEdit,1 as  UpMeter,iRoleID  
                                             from V_P_Admin where 1=1 and cAdminName='{0}' and cAdminPassWord='{1}'", name, pwd);
                dt = APP.SQLServer_PingTai.SelectDataTable(sql);
                if (dt.Rows.Count > 0)
                {
